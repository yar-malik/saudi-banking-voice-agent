# Saudi Banking Voice Agent

> A free Arabic voice agent for Saudi banks — card disputes, balances, transfers and fraud call-backs in Najdi Arabic, inside your own network.
...
<p align="center">
  <a href="https://voho.ai/industries/banking">
    <img src="docs/demo.png" alt="Saudi Banking Voice Agent — Card dispute played end to end in Saudi Arabic, with the tool calls and the outcome visible" width="900">
  </a>
</p>

<p align="center">
  <b><a href="https://voho.ai/industries/banking">▶ Play the live demo</a></b> — runs in your browser, no sign-up.
</p>
...
Queues full of balance checks and card disputes, while the calls that actually need a banker wait behind them.

This is for retail bank contact centres and digital banking teams. Every conversation is in **Najdi Arabic** — the
...
| Scenario | | |
| --- | --- | --- |
| `dispute` | Card dispute | A disputed transaction opened, provisionally credited and confirmed — without a queue. |
| `fraud` | Fraud call-back | Outbound verification on a suspicious transaction, before the customer notices. |
| `balance` | Balance and transfer | The simple calls that fill your queue all day, handled without a person. |

Play any of them right now, before you have connected anything:

```bash
python examples/play.py dispute
```

The tools (`authenticate_caller`, `block_card`, `fraud_signal`, `get_balance`, `open_dispute`, `transfer_funds`) replay the results recorded with each scenario, so the
...
| Scenario | | |
| --- | --- | --- |
| `kyc-docs` | KYC document pack | a hard document read into fields |
| `aml` | AML alert triage | an event carried across several systems |
| `rm-note` | Relationship manager notes | speech turned into business-ready text, then an action |
...
```bash
git clone https://github.com/yar-malik/saudi-banking-voice-agent.git
cd saudi-banking-voice-agent
...
@implement("authenticate_caller")
def authenticate_caller(checked: str) -> dict:
    # your system here
    return {"verified": true, "customer_id": "C-88213"}
```
...
In banking, that usually means:

| System | What it is for |
| --- | --- |
| **Nafath** | step-up authentication before anything is read out or moved |
| **mada** | the national card scheme behind most disputed transactions |
| **sarie** | SAMA's instant payment rails, for transfers raised on a call |
| **SADAD** | bill payments, the single most common reason the phone rings |
| **Your core banking system** | read and write through its own API, as a service account you scope |
