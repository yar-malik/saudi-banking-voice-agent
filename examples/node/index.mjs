...
const said = process.argv.slice(2).join(' ') || 'أبي أعرف رصيد حسابي، وفيه عملية ما أعرفها'
...
if (!agentId) {
  ...
  const audio = await voho('/v1/speech', { text: 'أهلاً بك في بنك النخبة، معك ليلى. كيف أقدر أساعدك؟', voice: 'layla', model: 'sada-1', format: 'mp3' }, true)
...
const out = await voho(`/v1/agents/${agentId}/reply`, {
  text: said,
  variables: { company: process.env.VOHO_COMPANY ?? 'بنك النخبة' },
})
