...
said = " ".join(sys.argv[1:]) or "أبي أعرف رصيد حسابي، وفيه عملية ما أعرفها"
...
if not agent_id:
    ...
    audio = voho("/v1/speech", {"text": "أهلاً بك في بنك النخبة، معك ليلى. كيف أقدر أساعدك؟", "voice": "layla", "model": "sada-1", "format": "mp3"}, raw=True)
...
out = voho("/v1/agents/%s/reply" % agent_id, {
    "text": said,
    "variables": {"company": os.environ.get("VOHO_COMPANY", "بنك النخبة")},
})
